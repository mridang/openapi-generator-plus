...
class HeaderSelector:
    ...
    HeaderData = namedtuple('HeaderData', ['header', 'weight'])
    ...
    def _get_accept_header_with_adjusted_weight(self, accept: List[str], headers_with_json: List[str]) -> str:
        ...
        for header in accept:
            ...
            if header_data.header.lower().startswith('application/json'):
        ...
        if with_application_json:
            accept_headers.extend(self._adjust_weight(with_application_json, current_weight, has_more_than_28_headers))
        if with_json:
            accept_headers.extend(self._adjust_weight(with_json, current_weight, has_more_than_28_headers))
        if without_json:
            accept_headers.extend(self._adjust_weight(without_json, current_weight, has_more_than_28_headers))

        return ','.join(accept_headers)

    def _get_header_and_weight(self, header: str):
        ...
        """
        matcher = self._WEIGHT_PATTERN.match(header)
        if matcher:
            header_value = matcher.group(1)
            weight = float(matcher.group(2))
            return self.HeaderData(header=header_value, weight=int(weight * 1000))
        else:
    ...
    def _adjust_weight(self, headers: List, current_weight: List[int], has_more_than_28_headers: bool) -> List[str]:
        ...
        """
        headers.sort(key=lambda h: h.weight, reverse=True)

        accept_headers: List[str] = []
        for index, header in enumerate(headers):
            if index > 0 and headers[index - 1].weight > header.weight:
                current_weight[0] = self.get_next_weight(current_weight[0], has_more_than_28_headers)

            weight = current_weight[0]
            accept_headers.append(self._build_accept_header(header.header, weight))
    ...
    def _build_accept_header(self, header: str, weight: int) -> str:
        ...
        weight_str = f'{weight / 1000:.3f}'.rstrip('0')
        if weight_str.endswith('.'):
            weight_str = weight_str[:-1]

        return clean_header + ';q=' + weight_str
    ...
    def get_next_weight(self, current_weight: int, has_more_than_28_headers: bool) -> int:
        ...
        return current_weight - int(10 ** math.floor(math.log10(current_weight - 1)))
