...
defmodule PetstoreClient.Api.BaseApi do
  ...
  def invoke_api_for_result(
    ...
    method =
      if is_atom(method),
        do: method |> Atom.to_string() |> String.downcase() |> String.to_atom(),
        else: method
    ...
    selected =
      PetstoreClient.HeaderSelector.select_headers(accepts, content_type || "", is_multipart)

    headers = %{}

    headers =
      if selected["Accept"], do: Map.put(headers, "Accept", selected["Accept"]), else: headers

    headers =
      if selected["Content-Type"],
        do: Map.put(headers, "Content-Type", selected["Content-Type"]),
        else: headers
      ...
      try do
        {:ok,
         case state.api_client do
           %{__struct__: mod} = client ->
             mod.send_request(client, method, url, headers, serialized_body)

           mod when is_atom(mod) ->
             mod.send_request(method, url, headers, serialized_body)
         end}
      rescue
        e -> {:error, e}
      end
