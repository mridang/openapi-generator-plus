    ...
    case File.read(path) do
      {:error, reason} ->
        raise PetstoreClient.ApiError,
          message:
            "failed to read CA certificate from #{inspect(path)}: #{to_string(:file.format_error(reason))}",
          status_code: 0
      ...
      {:ok, contents} ->
        case :public_key.pem_decode(contents) do
          [] ->
            raise PetstoreClient.ApiError,
              message:
                "failed to parse CA certificate from #{inspect(path)}: no PEM blocks found or unparseable",
              status_code: 0
  ...
  def send_request(%__MODULE__{} = client, method, url, headers, body, request_opts)
    ...
    response =
      try do
        ...
        e ->
          raise PetstoreClient.ApiError, message: Exception.message(e), status_code: 0, cause: e
      end
