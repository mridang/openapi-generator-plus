...
defmodule PetstoreClient.DefaultApiClient do
  ...
  defp validate_ca_cert_path(nil) do
    :ok
  end
    ...
    case File.read(path) do
      {:error, reason} ->
        raise PetstoreClient.ApiError,
          message: "failed to read CA certificate from #{inspect(path)}: #{to_string(:file.format_error(reason))}",
          status_code: 0
      ...
      {:ok, contents} ->
        case :public_key.pem_decode(contents) do
          [] ->
            raise PetstoreClient.ApiError,
              message: "failed to parse CA certificate from #{inspect(path)}: no PEM blocks found or unparseable",
              status_code: 0
