...
defmodule PetstoreClient.Auth.OAuth.OAuth2TokenManager do
  ...
  """
  @spec get_access_token(pid(), String.t(), %{optional(String.t()) => String.t()}, %{
  ...
  defp parse_oauth2_server_error(status_code, body) do
    ...
    case parsed do
      %{"error" => code} when is_binary(code) and code != "" ->
        description =
          if is_binary(parsed["error_description"]), do: parsed["error_description"], else: nil

        uri = if is_binary(parsed["error_uri"]), do: parsed["error_uri"], else: nil

        PetstoreClient.Auth.OAuth.OAuth2ServerError.exception(
