...
defmodule PetstoreClient.ObjectSerializer do
  ...
  def find_and_cast_into_type(type_name, data) do
    ...
    case type_str do
      "Boolean" ->
        if is_boolean(data), do: data, else: raise(PetstoreClient.SchemaMismatchError, message: "not a Boolean")

      "Float" ->
        if is_float(data), do: data, else: raise(PetstoreClient.SchemaMismatchError, message: "not a Float")

      "Integer" ->
        if is_integer(data), do: data, else: raise(PetstoreClient.SchemaMismatchError, message: "not an Integer")

      "String" ->
        if is_binary(data), do: data, else: raise(PetstoreClient.SchemaMismatchError, message: "not a String")

      "Object" ->
        if is_map(data), do: data, else: raise(PetstoreClient.SchemaMismatchError, message: "not an Object")
