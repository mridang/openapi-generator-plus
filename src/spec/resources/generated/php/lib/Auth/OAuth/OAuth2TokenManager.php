...
final class OAuth2TokenManager
{
    ...
    public function getAccessToken(string $tokenUrl, array $params, array $extraHeaders = []): string
    {
        ...
        if ($this->refreshToken !== null && $this->refreshToken !== '') {
            ...
            try {
                 ...
                 * Every SDK failure (OAuth2ServerException, OAuth2TokenException,
                 * NetworkException, ...) extends the SDK root. Fall back to
                 * re-running the original grant below. */
            }
        }
        $this->fetchToken($tokenUrl, $params, $extraHeaders);
        if ($this->accessToken === null) {
            throw new OAuth2TokenException('Failed to obtain access token');
        }
     ...
     * @throws OAuth2ServerException if the token endpoint returns an error response
     * @throws OAuth2TokenException if the token endpoint answers unusably
     */
    /**
     * @param array<string, string> $params       the request body parameters
    ...
    private function fetchToken(string $tokenUrl, array $params, array $extraHeaders = []): void
    {
         ...
         * transport via $noRedirect=true so the 3xx surfaces here and is
         * refused below. A transport failure propagates unchanged as
    ...
    private static function parseOAuth2ServerError(int $statusCode, string $body): OAuth2ServerException
    {
        ...
        if (is_array($parsed) && isset($parsed['error']) && is_string($parsed['error']) && $parsed['error'] !== '') {
            ...
            $uri = isset($parsed['error_uri']) && is_string($parsed['error_uri'])
                ? $parsed['error_uri']
                : null;
            return new OAuth2ServerException($statusCode, $parsed['error'], $description, $uri, $body);
        }
