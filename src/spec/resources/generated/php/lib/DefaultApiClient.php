...
class DefaultApiClient implements ApiClient
{
    ...
    public function sendRequest(string $method, string $url, array $headers, mixed $body): ApiResponse
    {
        ...
        try {
            ...
            if ($this->transportOptions->followRedirects) {
                ...
                $originalUrl = $url;
                $hops = 0;
                while ($hops < $maxRedirects && self::isRedirectStatus($response->getStatusCode())) {
                    ...
                    $redirectHeaders = $mergedHeaders;
                    if ($crossOrigin) {
                        foreach (array_keys($redirectHeaders) as $hname) {
                            $lower = strtolower((string) $hname);
                            if (in_array($lower, ['authorization', 'cookie', 'proxy-authorization'], true)) {
                    ...
                    $url = $nextUrl;
                    $response = $this->client->request($method, $nextUrl, $redirectOptions);
                    $hops++;
                }
            }
     ...
     */
    private static function isRedirectStatus(int $code): bool
    {
