...
test('exchange code rejects empty code', function (): void {
    /* exchangeCode must reject an empty / whitespace-only authorization
     * code before issuing the token POST, surfacing the caller bug as a
     * catchable InvalidArgumentException rather than a server-side
     * invalid_grant. */
    $client = makeOAuth2AuthCodeMockClient();
    $authenticator = new OAuth2AuthorizationCodeAuthenticator(
        'https://api.example.com',
...
test('exchange code rejects whitespace only code', function (): void {
     ...
     * issuing the token POST (trim($code) === '' guard). */
    $client = makeOAuth2AuthCodeMockClient();
    $authenticator = new OAuth2AuthorizationCodeAuthenticator(
        'https://api.example.com',
        'my-client-id',
...
test('authorize url with existing query string uses ampersand separator', function (): void {
    $authenticator = new OAuth2AuthorizationCodeAuthenticator(
        'https://api.example.com',
        'my-client-id',
        'my-client-secret',
