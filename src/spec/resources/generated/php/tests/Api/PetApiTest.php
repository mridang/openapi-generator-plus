...
test('find pets by status', function (): void {
    ...
    // Item-level deserialization (raw array → Pet instance) requires a
    // Symfony ObjectNormalizer extension to walk constructor args through
    // the normalizer chain; tracked as a separate follow-up. For now the
    // container is asserted; the items remain raw associative arrays.
...
test('add pet photos', function (): void {
    ...
    // Item-level deserialization tracked as separate follow-up; see
    // `find pets by status` test for the underlying Symfony limitation.
    expect($result)->toBeInstanceOf(\Ds\Vector::class);
    expect($result->count())->toBeGreaterThan(0);
    unlink($tmpFile);
});
