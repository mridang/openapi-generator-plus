...
public class OpenIdConnectAuthenticatorTest
{
    ...
    [Fact(
        Skip = "Feature gap: GetDelegateAsync parses response.Body without first "
            + "checking the HTTP status, so a non-2xx discovery response surfaces "
            + "as a JsonException (invalid JSON), not as a status error. Documented "
