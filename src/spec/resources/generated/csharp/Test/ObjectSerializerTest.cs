...
public class ObjectSerializerTest
{
    ...
    public class DateTimeOffsetPreservationTests
    {
        ...
        [Fact]
        public void SubsecondsPreservedInSerializedDatetime()
        {
            var dt = DateTimeOffset.Parse(
        ...
        /* Encoders previously truncated sub-second precision while the reader
         * accepted fractions, making the JSON round-trip lossy. The instant
         * 2020-01-02T03:04:05.123Z (UTC, millisecond precision) must serialize
         * WITH the ".123" fraction and deserialize back to the same instant.
         * Milliseconds (3 digits) are the cross-SDK common denominator every
         * native date-time type supports. */
        [Fact]
        public void DateTimeSerializationPreservesSubSecondPrecision()
        {
            var serializer = new ObjectSerializer();
            var instant = new DateTimeOffset(2020, 1, 2, 3, 4, 5, 123, TimeSpan.Zero);
            var metadata = new PhotoMetadata { TakenAt = instant };

            string json = serializer.Serialize(metadata);
            Assert.Contains(".123", json);

            var roundTripped = serializer.Deserialize<PhotoMetadata>(json);
            Assert.NotNull(roundTripped);
            Assert.Equal(instant, roundTripped!.TakenAt);
        }
    }
    ...
    public class RequiredFieldStrictnessTests
    {
        ...
        public void UnknownEnumValueThrows()
        {
            ...
            var json =
                "{\"id\":1,\"name\":\"Fido\",\"photoUrls\":[\"https://example.com/fido.jpg\"],\"status\":\"banana\"}";
            var ex = Assert.Throws<SerializationException>(() =>
    ...
    public class TimeOnlyAndDurationTests
    {
        ...
        [Theory]
        [InlineData("\"99999999999999999999s\"")] // does not fit a long
        [InlineData("\"999999999999s\"")] // fits a long, overflows TimeSpan ticks
        [InlineData("\"\u0661\u0662s\"")] // non-ASCII digits
        public void MalformedOrOverflowingDurationRaisesSerializationException(string json)
        {
            // Neither a raw FormatException/OverflowException nor a silently
            // wrapped tick count: the caller sees the SDK's own error.
            var ex = Assert.Throws<SerializationException>(() =>
                _serializer.Deserialize<TimeSpan>(json)
            );
            Assert.IsAssignableFrom<OpenAPIException>(ex);
        }
    }
    ...
    public class NonLowercaseStringEnumTests
    {
        ...
        {
            var item = _serializer.Deserialize<StockItem>(
        ...
        public void AvailabilityRejectsUnknownWireValue()
        {
            ...
            Assert.Throws<SerializationException>(() =>
                _serializer.Deserialize<StockItem>(
                    "{\"priority\":1,\"availability\":\"available\"}"
                )
            );
        }
    ...
    public class AdditionalPropertiesTopLevelTests
    {
        ...
        {
            var meta = _serializer.Deserialize<Metadata>(
                "{\"createdAt\":\"2024-01-01T00:00:00Z\",\"region\":\"emea\"}"
            );
            Assert.NotNull(meta);
    ...
    public class ByteArrayEqualityTests
    {
        ...
        )
        {
            return new BinaryVault
            {
                ...
                Labels =
                    labels ?? new Dictionary<string, byte[]> { ["first"] = new byte[] { 7, 8 } },
            };
        }
        ...
        public void DifferingMapValueCompareUnequal()
        {
            var left = VaultOf();
            var right = VaultOf(
                labels: new Dictionary<string, byte[]> { ["first"] = new byte[] { 7, 9 } }
            );
        ...
        public void DifferingMapKeyCompareUnequal()
        {
            var left = VaultOf();
            var right = VaultOf(
                labels: new Dictionary<string, byte[]> { ["second"] = new byte[] { 7, 8 } }
            );
