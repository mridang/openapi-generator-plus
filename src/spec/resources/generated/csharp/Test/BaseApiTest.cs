...
public class BaseApiTest
{
    ...
    // (e.g. to a request-reflecting echo endpoint). getInventory is declared
    ...
    public async Task SecurityNoneOperationSuppressesClientAuthenticator()
    {
        ...
        var config = new Configuration("http://localhost");
        var api = new StoreApi(client, config, clientAuth);

        try
        {
            // getInventory is security:[]; the captured request is what matters.
            await api.GetInventoryAsync();
        }
        catch (Exception)
        {
            // The captured request is asserted regardless of the response shape.
        }
    ...
    // INHERITED GLOBAL SECURITY (guard against over-suppression on inherited
    // ops): getPetById declares no security of its own, so it inherits the
    // global requirement. This generator strips the inherited auth methods off
    // such an operation, so the api method passes null (not the NoAuth
    // sentinel); BaseApi must then fall back to the client-level authenticator.
    ...
    [Fact]
    public async Task InheritedGlobalSecurityOperationAppliesClientAuthenticator()
    {
        var client = new CapturingApiClient();
        var clientAuth = new TestAuthenticator(
        ...
        try
        {
            // getPetById inherits the global security requirement; with no
            // per-call override the client authenticator must be applied.
        ...
        Assert.True(client.CapturedHeaders.ContainsKey("Authorization"),
            "inherited-security operation must carry the client Authorization header");
