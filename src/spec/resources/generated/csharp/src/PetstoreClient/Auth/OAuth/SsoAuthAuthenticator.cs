// Swagger Petstore - OpenAPI 3.0
// A simplified Pet Store API for integration testing.
//
// The version of the OpenAPI document: 1.0.0
//
// Generated by: https://openapi-generator.tech
#nullable enable

namespace PetstoreClient.Auth.OAuth;

/// <summary>
/// Scheme-specific authenticator for the <c>ssoAuth</c> security scheme.
/// </summary>
public sealed class SsoAuthAuthenticator : OpenIdConnectAuthenticator
{
    public SsoAuthAuthenticator(string host, string clientId, string clientSecret, Uri redirectUri)
        : base(
            host,
            new Uri("https://auth.example.com/.well-known/openid-configuration"),
            clientId,
            clientSecret,
            redirectUri,
            []
        ) { }
}
