...
public sealed class OAuth2TokenManager
{
    ...
    )
    {
        if (!string.IsNullOrEmpty(RefreshToken))
        {
                ...
                when (ex is not OutOfMemoryException and not StackOverflowException)
            {
                 ...
                 * refresh token (OAuth2ServerError), a 2xx refresh body missing
                 * access_token (OAuth2TokenError), a transport failure
    ...
    )
    {
         ...
         * 3xx we refuse to honour it. This is a client-side/transport refusal,
        ...
        if (response.StatusCode is >= 300 and < 400)
        {
            throw new OAuth2TokenError(
                $"Refusing to follow redirect from OAuth2 token endpoint {tokenUrl} "
                    + $"(status {response.StatusCode}); replaying client credentials "
                    + "to the redirect target would leak them."
            );
        }
...
public sealed class OAuth2TokenError : ZitadelException
{
...
public sealed class OAuth2ServerError : ZitadelException
{
