...
let package = Package(
    ...
    dependencies: [
        ...
        /* Test-only: the trace-context tests configure a real OpenTelemetry
           SDK with an in-memory exporter so they exercise the propagator with
           an active span instead of asserting the no-op path twice. */
