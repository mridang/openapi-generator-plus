...
public struct ISO8601DurationError: Error, LocalizedError {
...
public func parseISO8601Duration(_ input: String) throws -> TimeInterval {
    ...
    // Average Gregorian year / month lengths in seconds.
    ...
    func consumeNumber() throws -> Double {
        ...
        // Accept optional decimal point. Reject signs/exponents.
    ...
    for ch in s {
        ...
        if ch.isNumber || ch == "." || ch == "," {
            // Accept ',' as the alternate decimal mark per ISO-8601.
...
public func formatISO8601Duration(_ interval: TimeInterval) -> String {
    ...
    if !timeBody.isEmpty {
        ...
        // Non-zero interval shorter than a designator boundary —
        // should not happen given the math above, but keep the
        // output well-formed.
        out += "T0S"
    }
    return out
