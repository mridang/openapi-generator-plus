...
public class OAuth2PasswordAuthenticator: BaseAuthenticator, HttpAwareAuthenticator,
  @unchecked Sendable
{
  ...
  override public func authHeaders() async throws -> [String: String] {
    ...
    if clientAuthMethod == .basic {
      ...
      /* RFC 6749 §2.3.1 specifies application/x-www-form-urlencoded, in
       * which a space is encoded as `+`, not `%20`. Normalise after
       * percent-encoding so a credential containing a space matches the
       * other SDKs (Java/Kotlin/Go/Rust/Ruby/Elixir). */
      let encodedID =
        (clientID.addingPercentEncoding(withAllowedCharacters: unreserved) ?? clientID)
    ...
    /* Do NOT swallow the token-fetch error: a failed password-grant
     * exchange must surface to the caller rather than be collapsed into
     * an empty header map that would send the API request
     * unauthenticated. */
...
extension OAuth2PasswordAuthenticator: CustomStringConvertible, CustomDebugStringConvertible {
  /// A description that redacts the client secret and resource owner password
  /// so neither leaks through string interpolation, logging, or debugging
  /// output.
  public var description: String {
    "\(type(of: self))(host: \(_host), clientID: \(clientID), clientSecret: ***, tokenURL: \(tokenURL), refreshURL: \(refreshURL), username: \(username), password: ***, scopes: \(scopes), clientAuthMethod: \(clientAuthMethod))"
  }

  /// A debug description that redacts the client secret and resource owner
  /// password so neither leaks through string interpolation, logging, or
  /// debugging output.
