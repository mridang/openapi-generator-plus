...
open class BaseApi {
  ...
  func invokeAPI(_ params: InvokeAPIParams) async throws -> ApiResponse {
    ...
    if let auth = effectiveAuth {
      ...
      if !cookies.isEmpty {
        let cookieStr = cookies.map {
          let ek = $0.key.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? $0.key
          let ev =
    ...
    -> ApiResult<T>
  {
    ...
    var data: T? = nil
    if !response.body.isEmpty && headerSelector.isJsonMime(responseContentType) {
