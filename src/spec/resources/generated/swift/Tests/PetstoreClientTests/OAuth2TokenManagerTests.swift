...
@Suite final class OAuth2TokenManagerTests {
  ...
  @Test func testThrowsWhenNoApiClientInjected() async {
    ...
    } catch ConfigurationError.invalidState {
      /* A token requested before the ApiClient is injected is a wrong
         call order, not an API failure. */
    } catch {
      Issue.record("expected ConfigurationError.invalidState, got \(error)")
    }
  }
  ...
  @Test func testTokenEndpointFailuresAreTyped() async {
    let params = ["grant_type": "client_credentials"]
    let tokenURL = "https://auth.example.com/token"

    let notJson = MockApiClient()
    notJson.responses = [ApiHttpResponse(statusCode: 200, body: "<html>ok</html>", headers: [:])]
    var manager = OAuth2TokenManager()
    manager.setApiClient(notJson)
    await #expect(throws: OAuth2TokenError.self) {
      _ = try await manager.getAccessToken(tokenURL: tokenURL, params: params)
    }

    let unavailable = MockApiClient()
    unavailable.responses = [ApiHttpResponse(statusCode: 503, body: "down", headers: [:])]
    manager = OAuth2TokenManager()
    manager.setApiClient(unavailable)
    let serverError = await #expect(throws: OAuth2ServerError.self) {
      _ = try await manager.getAccessToken(tokenURL: tokenURL, params: params)
    }
    #expect(serverError?.statusCode == 503)

    let refused = MockApiClient()
    refused.failure = NetworkError(statusCode: 0, message: "connection refused")
    manager = OAuth2TokenManager()
    manager.setApiClient(refused)
    await #expect(throws: NetworkError.self) {
      _ = try await manager.getAccessToken(tokenURL: tokenURL, params: params)
    }

    let timedOut = MockApiClient()
    timedOut.failure = NetworkTimeoutError(statusCode: 0, message: "timed out")
    manager = OAuth2TokenManager()
    manager.setApiClient(timedOut)
    await #expect(throws: NetworkTimeoutError.self) {
      _ = try await manager.getAccessToken(tokenURL: tokenURL, params: params)
    }
  }
  ...
  @Test func testRefreshTokenEmptyStringPreservesExisting() async throws {
    ...
    client.responses.append(
      makeResponse(
        body: "{\"access_token\":\"old_access\",\"refresh_token\":\"old_refresh\",\"expires_in\":1}"
      ))
    client.responses.append(
      makeResponse(
        body: "{\"access_token\":\"new_access\",\"expires_in\":3600,\"refresh_token\":\"\"}"))
  ...
  @Test func testExpiresInAsJsonStringIsAccepted() async throws {
    ...
    let client = MockApiClient()
    client.responses.append(
      makeResponse(body: "{\"access_token\":\"str-tok\",\"expires_in\":\"3600\"}"))
  ...
  @Test func testExpiresInAsFloatIsFloored() async throws {
    ...
    let client = MockApiClient()
    client.responses.append(
      makeResponse(body: "{\"access_token\":\"flt-tok\",\"expires_in\":3600.5}"))
  ...
  @Test func testTokenRequestSetsNoRedirect() async throws {
    let client = MockApiClient()
    client.responses.append(
      makeResponse(body: "{\"access_token\":\"tok-no-redir\",\"expires_in\":3600}"))
  ...
  /// POST to the redirect target. Like any non-2xx answer, the refusal is an
  /// OAuth2ServerError carrying that status.
  ...
  func testTokenEndpointRedirectIsRejected(status: Int) async {
    ...
    let error = await #expect(throws: OAuth2ServerError.self) {
      _ = try await manager.getAccessToken(
        tokenURL: "https://auth.example.com/token",
  ...
  @Test func testInvalidateTriggersSingleRefetchUnderConcurrency() async throws {
    ...
    #expect(
      client.callCount == 2,
      "invalidate + concurrent callers must produce one refetch, got \(client.callCount)")
  }

  /// Gap 3.2: the redirect-refusal error should name the offending Location
  /// header for diagnostics. The Swift SDK refuses the redirect with an
  /// OAuth2ServerError that carries the status code and raw body only, not
  /// the Location target, so this scenario cannot be asserted without
  /// fabricating behaviour the SDK does not implement.
  @Test(
    .disabled(
      "Swift OAuth2TokenManager redirect-refusal error does not surface the Location header"))
  func testRedirectRefusalErrorIncludesLocation() async {
    ...
    do {
      ...
      Issue.record("Expected OAuth2ServerError for 307")
    } catch let error as OAuth2ServerError {
      #expect(
