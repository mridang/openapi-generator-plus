...
@Suite(.serialized) final class DefaultApiClientTests {
  ...
  /* Configuring a proxy throws on Linux, where URLSession has no
   * connectionProxyDictionary, so the proxy cases run on the Apple
   * platforms only. Everything else in this suite is platform-independent
   * and runs everywhere. */
  #if !os(Linux)
