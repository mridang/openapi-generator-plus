...
func NewBaseApi(apiClient ApiClient, config *Configuration) *BaseApi {
	if apiClient == nil {
...
func (b *BaseApi) invokeAPIForResult(params invokeAPIParams) (*HttpResponse, error) {
	...
	if params.auth != nil {
		authQueryParams := params.auth.QueryParams()
	...
	// Merge auth headers
	if params.auth != nil {
		for k, v := range params.auth.AuthHeaders() {
			headers[k] = v
		}
		// Handle cookie params
		cookies := params.auth.CookieParams()
		if len(cookies) > 0 {
