...
// access token, or the token-endpoint error. base_api prefers this method, so
// a failed token request surfaces as the real OAuth error instead of an
// unauthenticated request that fails later as a confusing 401.
func (a *OAuth2PasswordAuthenticator) AuthHeadersOrError() (map[string]string, error) {
...
// AuthHeaders returns the Bearer authentication header with a valid access
// token, or an empty map when the token request fails. Use
// AuthHeadersOrError to see why.
