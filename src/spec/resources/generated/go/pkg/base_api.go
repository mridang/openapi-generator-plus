...
// ErrMissingRequiredParameter is returned by an operation called without one
// of its required parameters, or with an empty path parameter. Match it with
// errors.Is.
var ErrMissingRequiredParameter = errors.New("missing required parameter")

// ErrInvalidCookie is returned when an authenticator supplies a cookie name or
// value containing characters RFC 6265 forbids. Match it with errors.Is.
...
func (b *baseApi) invokeApiForResult(params invokeApiParams) (*ApiHttpResponse, error) {
	...
	if effectiveAuth != nil {
		 ...
		 * pass through raw instead. Invalid input is a caller mistake and is
		 * returned as ErrInvalidCookie. */
		cookies := effectiveAuth.CookieParams()
		if len(cookies) > 0 {
			var cookieParts []string
			for k, v := range cookies {
				if !isValidCookieName(k) {
					return nil, fmt.Errorf("%w: cookie name '%s' contains characters forbidden by RFC 6265", ErrInvalidCookie, k)
				}
				if !isValidCookieValue(v) {
					return nil, fmt.Errorf("%w: cookie value for '%s' contains characters forbidden by RFC 6265", ErrInvalidCookie, k)
				}
