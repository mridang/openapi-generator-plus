...
func TestFromResponse_MapsEveryStatusToItsType(t *testing.T) {
	...
	cases := []struct {
		...
		{302, "*errors.ApiError"},
		/* A status outside 400-599 is not a ClientError or a ServerError: the
		 * 5xx arm stops at 599, so 600 yields the base *ApiError. */
		{600, "*errors.ApiError"},
	}
...
func TestPackageRoot_ReexportsTheErrorTree(t *testing.T) {
	...
	if !errors.As(petstore.FromResponse(404, nil, ""), &notFound) {
