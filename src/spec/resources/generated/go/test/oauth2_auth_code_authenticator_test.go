...
func (c *fakeAuthCodeClient) SendRequest(method, url string, headers map[string]string, body any) (*auth.ApiHttpResponse, error) {
	return c.SendRequestWithOptions(method, url, headers, body, nil)
}

func (c *fakeAuthCodeClient) SendRequestWithOptions(method, url string, headers map[string]string, body any, opts *auth.RequestOptions) (*auth.ApiHttpResponse, error) {
	_ = opts
	c.LastURL = url
	if body != nil {
...
func TestOAuth2AuthCode_ExchangeCodeRejectsWhitespaceOnlyCode(t *testing.T) {
	t.Parallel()
	// oauth-exchangecode-no-empty-code-guard: a whitespace-only code is distinct
	// from the empty-string case and must also be rejected (TrimSpace guard)
	// before any token POST is issued.
	authObj := createAuthCodeAuthenticator()
	err := authObj.ExchangeCode("   ")
	if err == nil {
		t.Fatalf("expected error for whitespace-only code")
	}
	if !errors.Is(err, oauth.ErrAuthCodeEmpty) {
		t.Errorf("expected ErrAuthCodeEmpty for whitespace-only code, got %v", err)
	}
}
...
func TestOAuth2AuthCodeRedactsSecret(t *testing.T) {
	t.Parallel()
	authObj := createAuthCodeAuthenticator()
