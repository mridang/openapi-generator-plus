...
func (c *fakeTokenClient) SendRequest(method, url string, headers map[string]string, body any) (*auth.ApiHttpResponse, error) {
	return c.SendRequestWithOptions(method, url, headers, body, nil)
}

func (c *fakeTokenClient) SendRequestWithOptions(method, url string, headers map[string]string, body any, opts *auth.RequestOptions) (*auth.ApiHttpResponse, error) {
	c.LastURL = url
	if opts != nil {
...
// 301/303/308) rather than silently falling through. Like any non-2xx answer,
// the refusal is an *OAuth2ServerError carrying that status.
func TestOAuth2TokenManager_RejectsRedirect(t *testing.T) {
	...
	for _, status := range []int{301, 302, 303, 307, 308} {
		...
		if serverErr.StatusCode() != status {
			t.Errorf("status %d: expected StatusCode %d, got %d", status, status, serverErr.StatusCode())
		}
	}
}
...
func (c *slowFakeTokenClient) SendRequest(method, url string, headers map[string]string, body any) (*auth.ApiHttpResponse, error) {
	return c.SendRequestWithOptions(method, url, headers, body, nil)
}

func (c *slowFakeTokenClient) SendRequestWithOptions(_, _ string, _ map[string]string, _ any, _ *auth.RequestOptions) (*auth.ApiHttpResponse, error) {
	atomic.AddInt32(&c.callCount, 1)
	if c.delay > 0 {
		time.Sleep(c.delay)
	}
	return &auth.ApiHttpResponse{StatusCode: 200, Body: c.body, Headers: map[string]string{}}, nil
}
...
// Token endpoint: a 2xx body that is not JSON is an *OAuth2TokenError, and a
// transport failure is returned unchanged as the *NetworkError /
// *NetworkTimeoutError it already is.
func TestOAuth2TokenManager_UnusableAnswersAndTransportFailuresAreTyped(t *testing.T) {
	t.Parallel()
	params := map[string]string{"grant_type": "client_credentials"}
	...
	manager = oauth.NewOAuth2TokenManager()
	manager.SetApiClient(newFakeTokenClient(fakeResponse{err: refused}))
	_, err = manager.GetAccessToken("https://auth.example.com/token", params)
	var netErr *apierrors.NetworkError
	if !errors.As(err, &netErr) || netErr.StatusCode() != 0 {
		t.Errorf("expected *NetworkError with status 0, got %T: %v", err, err)
	}

	timeout := apierrors.NewNetworkTimeoutError("request timed out", errors.New("deadline exceeded"))
	manager = oauth.NewOAuth2TokenManager()
	manager.SetApiClient(newFakeTokenClient(fakeResponse{err: timeout}))
	_, err = manager.GetAccessToken("https://auth.example.com/token", params)
	var timeoutErr *apierrors.NetworkTimeoutError
	if !errors.As(err, &timeoutErr) {
		t.Errorf("expected *NetworkTimeoutError, got %T: %v", err, err)
	}
...
func (c *numberingSlowTokenClient) SendRequest(method, url string, headers map[string]string, body any) (*auth.ApiHttpResponse, error) {
	return c.SendRequestWithOptions(method, url, headers, body, nil)
}

func (c *numberingSlowTokenClient) SendRequestWithOptions(_, _ string, _ map[string]string, _ any, _ *auth.RequestOptions) (*auth.ApiHttpResponse, error) {
	n := atomic.AddInt32(&c.callCount, 1)
	if c.delay > 0 {
		time.Sleep(c.delay)
	}
	body := `{"access_token":"tok` + string(rune('0'+n)) + `","expires_in":3600}`
	return &auth.ApiHttpResponse{StatusCode: 200, Body: body, Headers: map[string]string{}}, nil
}
