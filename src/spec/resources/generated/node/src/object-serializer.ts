...
export class ObjectSerializer {
  ...
  static deserialize<T>(json: unknown, cls: ClassConstructor<T>): T {
    try {
      // Handle oneOf/anyOf composed schema classes
      const anyOfSchemas = (cls as Record<string, unknown>)['ANY_OF_SCHEMAS'] as string[] | undefined;
      const oneOfSchemas = (cls as Record<string, unknown>)['ONE_OF_SCHEMAS'] as string[] | undefined;
      const discProp = (cls as Record<string, unknown>)['DISCRIMINATOR_PROPERTY'] as string | undefined;
      const discMapping = (cls as Record<string, unknown>)['DISCRIMINATOR_MAPPING'] as
        | Record<string, string>
        | undefined;
      const schemas = anyOfSchemas ?? oneOfSchemas;

      if (schemas && json && typeof json === 'object') {
        // With discriminator, use the mapping directly
        ...
        // Without discriminator, try each schema
        for (const schemaName of schemas) {
          ...
          if (schemaCls) {
            try {
              ...
              // Check that at least one property was populated
