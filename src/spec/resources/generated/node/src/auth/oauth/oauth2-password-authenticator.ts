...
export class OAuth2PasswordAuthenticator implements HttpAwareAuthenticator {
  ...
  /* Redact the client secret and resource-owner password from the default
   * string/inspect representation so `console.log(auth)` / `util.inspect(auth)`
   * / JSON.stringify never exfiltrate the credentials into application logs. */
  [Symbol.for('nodejs.util.inspect.custom')](): string {
    return `OAuth2PasswordAuthenticator(host=${this.host}, clientId=${this.clientId}, username=${this.username}, clientSecret=***, password=***)`;
  }
