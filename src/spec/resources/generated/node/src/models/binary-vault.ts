...
export class BinaryVault {
  ...
  constructor(data?: Partial<BinaryVault>) {
     ...
     */
    if (this.seal != null && typeof this.seal === "string") {
      this.seal = Buffer.from(
        this.seal as unknown as string,
        "base64",
      ) as unknown as Buffer;
    } else if (this.seal != null && !Buffer.isBuffer(this.seal)) {
      throw new TypeError(
        `seal must be a Buffer or base64 string, got ${typeof this.seal}`,
      );
    }
    if (
      this.shards != null &&
      !Array.isArray(this.shards) &&
      !((this.shards as unknown) instanceof Set)
    ) {
      throw new TypeError(`shards must be an array, got ${typeof this.shards}`);
    }
    /**
     ...
     * non-string elements.
     */
    if (this.shards != null && Array.isArray(this.shards)) {
      this.shards = (this.shards as unknown as unknown[]).map((__e) => {
        if (typeof __e === "string") {
          return Buffer.from(__e, "base64");
        }
        if (__e != null && !Buffer.isBuffer(__e)) {
          throw new TypeError(
            `shards elements must be a Buffer or base64 string, got ${typeof __e}`,
          );
        }
        return __e;
      }) as unknown as Buffer[];
  ...
  equals(other: unknown): boolean {
    ...
    if (!(other instanceof BinaryVault)) {
      return false;
    }
    if (!BinaryVault.__deepEquals(this.seal, (other as BinaryVault).seal)) {
      return false;
    }
    if (!BinaryVault.__deepEquals(this.shards, (other as BinaryVault).shards)) {
      return false;
    }
    if (!BinaryVault.__deepEquals(this.labels, (other as BinaryVault).labels)) {
      return false;
    }
    return true;
  }
  ...
  private static __deepEquals(a: unknown, b: unknown): boolean {
    ...
    if (Array.isArray(a) && Array.isArray(b)) {
      ...
      return a.every((v, i) => BinaryVault.__deepEquals(v, b[i]));
    }
    if (a instanceof Set && b instanceof Set) {
      ...
      const bvals = [...b];
      return [...a].every((v) =>
        bvals.some((w) => BinaryVault.__deepEquals(v, w)),
      );
    }
    if (a instanceof Map && b instanceof Map) {
      ...
      for (const [k, v] of a) {
        if (!b.has(k) || !BinaryVault.__deepEquals(v, b.get(k))) {
          return false;
        }
      }
      return true;
    ...
    if (typeof a === "object" && typeof b === "object") {
      ...
      return ak.every(
        (k) =>
          ...
          BinaryVault.__deepEquals(ar[k], br[k]),
      );
    }
    return false;
  }
