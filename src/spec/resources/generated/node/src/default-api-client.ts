 ...
 * `Cookie`, `Proxy-Authorization`) plus the common API-key header
 * names emitted by `ApiKeyAuthenticator` (and observed across major
 * SaaS providers). undici's automatic redirect follower only strips
 * `Authorization`-style headers; leaving API-key headers attached
 * would leak the key to attacker-controlled hosts via a malicious
 * 302/307 from an upstream that supports both bearer and api-key
 * auth. Match is case-insensitive; entries here MUST be lowercase.
 */
...
export class DefaultApiClient implements ApiClient {
  ...
  async sendRequest(
    ...
    try {
      ...
      if (this.transportOptions.followRedirects && !noRedirect) {
        ...
        while (hops < maxRedirects && DefaultApiClient.isRedirectStatus(response.status)) {
          const location = response.headers.get('location');
          if (location == null || location.length === 0) break;
          const nextUrl = new URL(location, currentUrl).toString();
          const nextParsed = new URL(nextUrl);
          if (nextParsed.protocol !== 'http:' && nextParsed.protocol !== 'https:') break;
          ...
          for (const [k, v] of Object.entries(currentHeaders)) {
            if (crossOrigin && SENSITIVE_HEADER_NAMES.has(k.toLowerCase())) continue;
            if (
              nextBody === undefined &&
              (k.toLowerCase() === 'content-type' || k.toLowerCase() === 'content-length')
            ) {
              continue;
            }
            redirectHeaders[k] = v;
          ...
          response = await fetch(nextUrl, redirectOptions as RequestInit);
          hops++;
        }
      }
    } catch (error) {
  ...
  private static sameOrigin(a: string, b: string): boolean {
    try {
      ...
      const portA = ua.port !== '' ? ua.port : ua.protocol === 'https:' ? '443' : '80';
      const portB = ub.port !== '' ? ub.port : ub.protocol === 'https:' ? '443' : '80';
      return portA === portB;
    } catch {
      return false;
    }
