      ...
      if (this.transportOptions.proxy) {
        ...
        if (isHttps) {
          ...
          connectReq.on('connect', (_res, socket) => {
            // Wrap the tunnel socket with TLS manually so that
            // http.request never attempts DNS resolution on the
            // target hostname (which is only reachable via the proxy).
            ...
            if (this.transportOptions.timeout != null) {
              reqOptions.timeout = this.transportOptions.timeout;
            }
