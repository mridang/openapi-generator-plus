...
export abstract class BaseApi {
  ...
  protected async invokeApiForResult<T>(
    ...
    if (path.startsWith('http://') || path.startsWith('https://')) {
      ...
      const base = path.startsWith('/') ? this.config.baseUrl.replace(/\/+$/, '') : this.config.baseUrl;
      url = base + path;
    }
   ...
   */
  private serializeBody(body: unknown, contentType: string): string | Buffer | Record<string, unknown> | null {
    if (body == null) {
      return null;
    }
