...
describe('SENSITIVE_HEADER_NAMES allowlist', () => {
  ...
  it('includes common API-key header names', () => {
    expect(SENSITIVE_HEADER_NAMES.has('x-api-key')).toBe(true);
    expect(SENSITIVE_HEADER_NAMES.has('api-key')).toBe(true);
    expect(SENSITIVE_HEADER_NAMES.has('apikey')).toBe(true);
    expect(SENSITIVE_HEADER_NAMES.has('x-auth-token')).toBe(true);
    expect(SENSITIVE_HEADER_NAMES.has('x-access-token')).toBe(true);
    expect(SENSITIVE_HEADER_NAMES.has('x-functions-key')).toBe(true);
    expect(SENSITIVE_HEADER_NAMES.has('x-amz-security-token')).toBe(true);
    expect(SENSITIVE_HEADER_NAMES.has('ocp-apim-subscription-key')).toBe(true);
  });
...
describe('cross-origin redirect strips API-key headers', () => {
  ...
  it('strips Authorization, X-Api-Key, and Cookie on cross-origin 302', async () => {
    ...
    await client.sendRequest(
      ...
      `${srcUrl}/start`,
      {
        ...
        'X-Api-Key': 'k-12345',
        Cookie: 'session=xyz',
        'X-Functions-Key': 'azure-func-key',
    ...
    expect(lastReceivedHeaders['x-functions-key']).toBeUndefined();
    expect(lastReceivedHeaders['ocp-apim-subscription-key']).toBeUndefined();
