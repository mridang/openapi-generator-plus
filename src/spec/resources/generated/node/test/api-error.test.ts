...
describe("ApiError", () => {
  ...
  describe("getTypedErrorBody", () => {
    test("deserializes the raw body into the requested class", () => {
      ...
      const err = ApiError.fromResponse(400, {}, '{"id":42,"name":"Dogs"}');
  ...
  describe("error tree", () => {
    ...
    test("the OAuth2 errors are rooted at the branded base, not at ApiError", () => {
      const server = new OAuth2ServerError(
        400,
        "invalid_grant",
        null,
        null,
      ...
      expect(server).toBeInstanceOf(OpenAPIError);
      expect(token).toBeInstanceOf(OpenAPIError);
      expect(server).not.toBeInstanceOf(ApiError);
      expect(token).not.toBeInstanceOf(ApiError);
      expect(new SerializationError("boom")).toBeInstanceOf(OpenAPIError);
    });
  });
