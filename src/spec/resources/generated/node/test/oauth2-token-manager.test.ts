...
describe('OAuth2TokenManager', () => {
  ...
  test('missing access_token in 2xx response throws typed OAuth2TokenError', async () => {
    ...
    mockClient.responseBody = JSON.stringify({ refresh_token: 'x' });
  ...
  test('server error response parsed to typed OAuth2ServerError', async () => {
    ...
    const promise = tokenManager.getAccessToken('https://auth.example.com/token', {
      grant_type: 'client_credentials'
    });
    ...
    await promise.catch((err: OAuth2ServerError) => {
      ...
      expect(err.uri).toBe('https://docs.example.com/errors/invalid_grant');
    });
  });
