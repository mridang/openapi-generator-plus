...
describe("SecurityNoneSuppressesClientAuth", () => {
   ...
   *
   * `getPetById` is declared `security: []`, so it is the canonical
  ...
  test("security:[] op suppresses a configured api-key (header, query, cookie)", async () => {
    ...
    headerClient.responseBody = '{"id":1,"name":"Fido"}';
    ...
    const headerApi = new PetApi(
      headerClient,
      new Configuration({ baseUrl: "http://localhost" }),
      headerAuth,
    );
    try {
      await headerApi.getPetById(1);
    } catch {
      // capture-only
    }
    ...
    queryClient.responseBody = '{"id":1,"name":"Fido"}';
    ...
    const queryApi = new PetApi(
      queryClient,
      new Configuration({ baseUrl: "http://localhost" }),
      queryAuth,
    );
    try {
      await queryApi.getPetById(1);
    } catch {
      // capture-only
    }
    ...
    cookieClient.responseBody = '{"id":1,"name":"Fido"}';
    ...
    const cookieApi = new PetApi(
      cookieClient,
      new Configuration({ baseUrl: "http://localhost" }),
      cookieAuth,
    );
    try {
      await cookieApi.getPetById(1);
    } catch {
      // capture-only
    }
