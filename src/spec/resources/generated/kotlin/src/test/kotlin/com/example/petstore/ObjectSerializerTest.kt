...
class ObjectSerializerTest {
    ...
    inner class FixtureModelRegressionTests {
        ...
        fun integerBackedEnumSerializesAsNumber() {
            ...
            val item =
                com.example.petstore.models
                    .StockItem(priority = com.example.petstore.models.Priority.NUMBER_2)
            ...
            val decoded = serializer.deserialize<com.example.petstore.models.StockItem>("{\"priority\":2}")
            assertNotNull(decoded)
            assertEquals(com.example.petstore.models.Priority.NUMBER_2, decoded!!.priority)
        }
        ...
        fun nonLowercaseStringEnumPreservesWireCasing() {
            ...
            val available =
                serializer.serialize(
                    com.example.petstore.models.StockItem(
                        priority = com.example.petstore.models.Priority.NUMBER_1,
                        availability = com.example.petstore.models.Availability.AVAILABLE,
                    ),
                )
            ...
            val onHold =
                serializer.serialize(
                    com.example.petstore.models.StockItem(
                        priority = com.example.petstore.models.Priority.NUMBER_1,
                        availability = com.example.petstore.models.Availability.ON_HOLD,
                    ),
                )
            ...
            val decoded =
                serializer.deserialize<com.example.petstore.models.StockItem>(
                    "{\"priority\":1,\"availability\":\"Sold\"}",
                )
            assertNotNull(decoded)
            assertEquals(com.example.petstore.models.Availability.SOLD, decoded!!.availability)
        }
        ...
        fun referencedEnumFieldRejectsUnknownValue() {
            ...
            assertThrows(SerializationException::class.java) {
                serializer.deserialize<com.example.petstore.models.EdgeCases>("{\"favoriteColor\":\"purple\"}")
            }
            // Control: a declared value still decodes.
            val ok = serializer.deserialize<com.example.petstore.models.EdgeCases>("{\"favoriteColor\":\"green\"}")
            assertNotNull(ok)
            assertEquals(com.example.petstore.models.Color.GREEN, ok!!.favoriteColor)
        }
        ...
        fun byteFieldsRoundTripThroughBase64() {
            ...
            val passport =
                com.example.petstore.models.PetPassport(
                    thumbnail = thumb,
            ...
            val decoded = serializer.deserialize<com.example.petstore.models.PetPassport>(json)
            assertNotNull(decoded)
        ...
        fun doubleFieldAcceptsIntegralJsonValue() {
            ...
            val loc =
                serializer.deserialize<com.example.petstore.models.PhotoMetadataLocation>(
                    "{\"lat\":5,\"lng\":-122}",
                )
            assertNotNull(loc)
        ...
        fun additionalPropertiesRoundTripAtTopLevel() {
            ...
            val decoded =
                serializer.deserialize<com.example.petstore.models.Metadata>(
        ...
        fun nestedContainerDeepRoundTrips() {
            ...
            val item =
                com.example.petstore.models.StockItem(
                    priority = com.example.petstore.models.Priority.NUMBER_3,
                    matrix = listOf(listOf(1, 2), listOf(3, 4, 5)),
                )
            val json = serializer.serialize(item)
            val decoded = serializer.deserialize<com.example.petstore.models.StockItem>(json)
            assertNotNull(decoded)
        ...
        fun durationRoundTripsNegativeAndSmallMagnitude() {
            ...
            val negJson =
                serializer.serialize(
                    com.example.petstore.models
                        .EdgeCases(retryAfter = negative),
                )
            assertTrue(negJson.contains("\"retryAfter\":\"-90s\""), "negative duration must keep its sign, got: $negJson")
            val negDecoded = serializer.deserialize<com.example.petstore.models.EdgeCases>(negJson)
            ...
            val tinyJson =
                serializer.serialize(
                    com.example.petstore.models
                        .EdgeCases(retryAfter = tiny),
                )
            assertFalse(tinyJson.contains("E"), "duration must not use scientific notation, got: $tinyJson")
            assertFalse(tinyJson.contains("e-"), "duration must not use scientific notation, got: $tinyJson")
            val tinyDecoded = serializer.deserialize<com.example.petstore.models.EdgeCases>(tinyJson)
    ...
    /*
     * Byte-array equality across the three shapes a property can take. Kotlin's
     * synthesized data-class equals/hashCode compare ByteArray by REFERENCE
     * identity, and a List<ByteArray> bottoms out in reference-equal elements,
     * so a model holding bytes would report two instances carrying the same
     * content as unequal — and would misbehave as a map or set key. The
     * generated overrides must route every byte-array shape through
     * contentEquals/contentHashCode.
     *
     * The MAP case is the one that regressed: the template once selected the
     * LIST branch for a map and emitted a Collection call (zip/size) on a Map,
     * which did not compile; the fix that restored compilation then silently
     * fell back to plain == on the values, i.e. reference identity again. A
     * test that only covered the bare array and the list passed throughout
     * both defects, which is why the map case is asserted explicitly below —
     * both for a differing VALUE (content comparison) and a differing KEY (the
     * map's own structural comparison).
     *
     * The fixture model carries all three deliberately: a bare ByteArray, a
     * list of them, and a map of them.
     */
    @Nested
    @DisplayName("generated equality compares array content, not identity")
    inner class ByteArrayEquality {
        // Default arguments are re-evaluated on every call, so each invocation
        // hands back a vault whose arrays are FRESH objects. That is the whole
        // point: identity comparison cannot pass the first test below.
        private fun vaultOf(
            seal: ByteArray = byteArrayOf(1, 2, 3),
            shards: List<ByteArray> = listOf(byteArrayOf(4, 5), byteArrayOf(6)),
            labels: Map<String, ByteArray> = mapOf("first" to byteArrayOf(7, 8)),
        ): com.example.petstore.models.BinaryVault =
            com.example.petstore.models
                .BinaryVault(seal = seal, shards = shards, labels = labels)

        // The generated properties are nullable (`Map<String, ByteArray>?`),
        // so reading one back for an assertion needs a null check first. Assert
        // rather than silently default: a null here would mean the fixture
        // stopped populating the property and the test below would be vacuous.
        private fun labelsOf(vault: com.example.petstore.models.BinaryVault): Map<String, ByteArray> {
            val labels = vault.labels
        ...
        fun equalContentDistinctArraysCompareEqual() {
            val left = vaultOf()
            val right = vaultOf()
            ...
            assertNotSame(labelsOf(left)["first"], labelsOf(right)["first"])
        ...
        fun differingBareArrayCompareUnequal() {
            val left = vaultOf()
            val right = vaultOf(seal = byteArrayOf(1, 2, 4))
        ...
        fun differingListElementCompareUnequal() {
            val left = vaultOf()
            val right = vaultOf(shards = listOf(byteArrayOf(4, 5), byteArrayOf(9)))
        ...
        fun differingMapValueCompareUnequal() {
            val left = vaultOf()
            val right = vaultOf(labels = mapOf("first" to byteArrayOf(7, 9)))

            // Reference-identity comparison of the map values would call these
            // EQUAL: the two maps have the same key set, and Map's own equals
            // compares values with ==, which for ByteArray stops at the array
            // reference and never looks at the bytes. Only a per-value
            // contentEquals distinguishes them.
            assertNotEquals(left, right)
        }

        @Test
        @DisplayName("a differing map KEY makes them unequal")
        fun differingMapKeyCompareUnequal() {
            val left = vaultOf()
            val right = vaultOf(labels = mapOf("second" to byteArrayOf(7, 8)))
            ...
            // looked up by the WRONG map's keys) would miss this.
            assertNotEquals(left, right)
        }
    }
}
