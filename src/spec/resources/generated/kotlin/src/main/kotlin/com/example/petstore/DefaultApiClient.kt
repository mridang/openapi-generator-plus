...
class DefaultApiClient : ApiClient {
    ...
    constructor(transportOptions: TransportOptions) {
        ...
        if (!transportOptions.verifySsl) {
            ...
            val caCert = java.io.FileInputStream(transportOptions.caCertPath).use { fis ->
    ...
    private fun decompressBody(data: ByteArray, encoding: String): String {
        if (data.isEmpty()) return ""
        val decompressed = when (encoding.lowercase()) {
            "gzip", "x-gzip" ->
    ...
    private fun addMultipartField(
        ...
        when (value) {
            is File -> {
                val mediaType = java.nio.file.Files.probeContentType(value.toPath())
            ...
            is String, is Number, is Boolean -> {
                ...
                val json = kotlinx.serialization.json.Json.encodeToString(
                    kotlinx.serialization.serializer<Any>(), value as Any
                )
                builder.addFormDataPart(
                    fieldName, null,
 ...
 * returned as-is.
 */
private class MaxRedirectsInterceptor(private val maxRedirects: Int) : Interceptor {
