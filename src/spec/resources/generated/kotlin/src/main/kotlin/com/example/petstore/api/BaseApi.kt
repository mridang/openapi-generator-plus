...
abstract class BaseApi {
    ...
    protected suspend fun invokeApi(
        ...
        var url = if (path.startsWith("http://") || path.startsWith("https://")) {
            path
        } else {
            config.baseUrl + path
        }
        ...
        val requestBody: Any? = if (body != null) {
            when {
                body is ByteArray
    ...
    protected suspend inline fun <reified T> invokeApiForResult(
        ...
        val data: T? = if (response.body.isNotEmpty()) {
    ...
    private fun throwApiException(response: ApiResponse): Nothing {
        ...
        val errorBody: Any? = if (body.isNotEmpty()) {
            try {
