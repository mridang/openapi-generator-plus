...
abstract class AbstractApiClient implements ApiClient {
  ...
  /// Platform seam: whether [error], raised while sending the request or
  /// reading the response body, means no HTTP response arrived (connection
  /// refused, DNS failure, TLS failure, connection reset). The native
  /// transport recognises the `dart:io` socket and TLS errors; the web
  /// transport only ever sees them wrapped in `http.ClientException`.
  bool isNetworkError(Object error) => false;

  /// Matches the wording `dart:io` uses when a deadline expires, so the
  /// connect deadline is classified the same way the total one is.
  static final RegExp _timedOutMessage = RegExp(
    r'timed? ?out',
    caseSensitive: false,
  );

  /// Whether [message] describes an expired deadline rather than another
  /// transport failure.
  ///
  /// The client sets two deadlines: `HttpClient.connectionTimeout` on the
  /// connect phase and `Future.timeout` on the whole request. Only the
  /// second throws `dart:async`'s TimeoutException; the connect deadline
  /// throws a SocketException that `package:http` rewraps as a
  /// ClientException, so the same expired deadline surfaced as
  /// NetworkException or NetworkTimeoutException depending on which timer
  /// fired first. It is a timeout either way.
  static bool isTimeoutMessage(String message) =>
      _timedOutMessage.hasMatch(message);
  ...
  Future<ApiHttpResponse> sendRequest(
    ...
    try {
       ...
       * NetworkException (an ApiException with status 0) -- unless the
       * connect deadline is what expired, which is a timeout. */
      if (isTimeoutMessage(e.message)) {
        throw NetworkTimeoutException(message: e.message, underlyingError: e);
      }
       ...
       * why the SDK type is NetworkTimeoutException and not a name that
       * would clash with it. */
      ...
      if (isNetworkError(e)) {
        if (isTimeoutMessage(e.toString())) {
          throw NetworkTimeoutException(
            message: e.toString(),
            underlyingError: e,
          );
