...
void main() {
  group('DefaultApiClient unit tests', () {
     ...
     * connect phase and Future.timeout on the whole request. Only the second
     * throws dart:async's TimeoutException; the connect deadline throws a
     * SocketException that package:http rewraps as a ClientException, so the
     * same expired deadline used to surface as NetworkException or
     * NetworkTimeoutException depending on which timer fired first. It is a
     * timeout either way. */
    test(
      'an expired connect deadline throws NetworkTimeoutException',
      () async {
        final client = DefaultApiClient(
          transportOptions: TransportOptionsBuilder().timeout(200).build(),
        );
        await expectLater(
          // 192.0.2.0/24 (RFC 5737 TEST-NET-1) is unrouted, so the TCP
          // handshake never completes and a connect deadline is what expires.
          client.sendRequest('GET', 'http://192.0.2.1/never', {}, null),
          throwsA(
    ...
    test('a connect-timeout message is classified as a timeout', () {
      // The wording dart:io uses when HttpClient.connectionTimeout expires.
      expect(
        AbstractApiClient.isTimeoutMessage(
          'HTTP connection timed out after 0:00:00.200000, '
          'host: 192.0.2.1, port: 80',
        ),
        isTrue,
      );
      expect(AbstractApiClient.isTimeoutMessage('Connection refused'), isFalse);
    });
