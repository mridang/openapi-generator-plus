...
describe PetstoreClient::ValueSerializer do
  ...
  describe 'query location' do
    ...
    it 'array joins with comma for csv' do
      _(PetstoreClient::ValueSerializer.serialize(%w[a b c], :query, 'array', collection_format: :csv)).must_equal('a,b,c')
    end

    it 'array joins with space for ssv' do
      _(PetstoreClient::ValueSerializer.serialize(%w[a b c], :query, 'array', collection_format: :ssv)).must_equal('a b c')
    end

    it 'array joins with tab for tsv' do
      _(PetstoreClient::ValueSerializer.serialize(%w[a b c], :query, 'array', collection_format: :tsv)).must_equal("a\tb\tc")
    end

    it 'array joins with pipe for pipes' do
      _(PetstoreClient::ValueSerializer.serialize(%w[a b c], :query, 'array', collection_format: :pipes)).must_equal('a|b|c')
    end

    it 'array returns list for multi' do
      _(PetstoreClient::ValueSerializer.serialize(%w[a b c], :query, 'array', collection_format: :multi)).must_equal(%w[a b c])
    end
