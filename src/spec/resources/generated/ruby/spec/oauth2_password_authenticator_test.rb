...
class OAuth2PasswordAuthenticatorTest < Minitest::Test
  ...
  def test_sends_grant_type
    @mock_client.add_response(
      status_code: 200,
      body: JSON.generate({ 'access_token' => 'pw_token', 'expires_in' => 3600 })
    )

    headers = @authenticator.auth_headers

    assert_equal 'Bearer pw_token', headers['Authorization']
    ...
    assert_equal 'password', parsed_body['grant_type']
  end
  ...
  def test_sends_username_and_password
    ...
    assert_equal 'testpass', parsed_body['password']
