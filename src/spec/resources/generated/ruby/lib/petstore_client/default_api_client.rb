...
module PetstoreClient
  ...
  class DefaultApiClient < ApiClient # rubocop:disable Metrics/ClassLength
    ...
    def send_request(method, url, headers, body) # rubocop:disable Metrics/AbcSize,Metrics/CyclomaticComplexity,Metrics/MethodLength,Metrics/PerceivedComplexity
      ...
      begin
        ...
        if @transport_options.follow_redirects
          ...
          original_url = url
          hops = 0
          while hops < max_redirects && self.class.redirect_status?(response.status)
            location = response.headers['location']
            break if location.nil? || location.empty?

            next_url = self.class.resolve_url(url, location)
            break if next_url.nil?

            next_uri = URI.parse(next_url) rescue nil
            break unless next_uri && %w[http https].include?(next_uri.scheme)

            cross_origin = !self.class.same_origin?(original_url, next_url)
            redirect_headers = merged.dup
            if cross_origin
            ...
            url = next_url
            response = connection.run_request(method.downcase.to_sym, next_url, serialized_body, redirect_headers)
            hops += 1
          end
        end
    ...
    def self.same_origin?(a, b) # rubocop:disable Metrics/AbcSize
      ua = URI.parse(a) rescue nil
      ub = URI.parse(b) rescue nil
      return false if ua.nil? || ub.nil?
      return false unless ua.scheme&.downcase == ub.scheme&.downcase
      return false unless ua.host&.downcase == ub.host&.downcase

      port_a = ua.port || (ua.scheme == 'https' ? 443 : 80)
      port_b = ub.port || (ub.scheme == 'https' ? 443 : 80)
      port_a == port_b
    end
