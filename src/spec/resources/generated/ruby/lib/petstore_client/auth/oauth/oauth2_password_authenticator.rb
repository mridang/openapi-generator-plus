  ...
  module Auth
    module OAuth
      ...
      class OAuth2PasswordAuthenticator < BaseAuthenticator
        ...
        # Mask the client secret and resource-owner password so they never
        # leak through the default +inspect+/+p+/string-interpolation, which
        # would otherwise dump every instance variable verbatim.
        # @return [String]
        def inspect
          "#<#{self.class.name} host=#{@host.inspect} client_id=#{@client_id.inspect} " \
            'client_secret="***" username=' \
            "#{@username.inspect} password=\"***\">"
        end
        alias to_s inspect
