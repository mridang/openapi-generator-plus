...
module PetstoreClient
  ...
  module Models
    # A oneOf without a discriminator whose two primitive variants overlap: an integer literal validates against BOTH the number(float) and the integer variant under pydantic's lenient coercion. Used to verify first-match (not multiple-match-error) semantics for cross-SDK parity.
    module AmbiguousShape
      class << self
        ...
        def build(data)
          ...
          raise ArgumentError, 'JSON did not match any schema in the AmbiguousShape oneOf union'
        end
      end
    end
  end
