...
describe PetstoreClient::DefaultApiClient do
  ...
  describe 'redirect handling' do
    ...
    it 'replays multipart body across 307 redirects (T-new-3)' do
      ...
      # wiremock's bodyPatterns matches the replayed request body; 200
      # is returned only when the multipart parts arrive intact.
      response = client.send_request(:POST, "#{wiremock_url}/api/redirect-307-multipart", headers, multipart_body)

      _(response.status_code).must_equal(200)
      json = JSON.parse(response.body)
      _(json['replayed']).must_equal(true)
    end
  end
