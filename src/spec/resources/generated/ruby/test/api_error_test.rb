...
describe Petstore::Client::ApiError do
  parallelize_me!
  ...
  it 'is a StandardError subclass with a message' do
    err = Petstore::Client::ApiError.new(status_code: 500, message: 'boom')
  ...
  # branded root (::Petstore::Client::OpenAPIError) so a single rescue catches them all.
  ...
  it 'defaults status_code to 0 when no HTTP response was received' do
    _(Petstore::Client::ApiError.new('connection reset').status_code).must_equal(0)
  end

  it 'roots SerializationError at the branded base' do
    err = Petstore::Client::SerializationError.new('boom')

    _(err).must_be_kind_of(::Petstore::Client::OpenAPIError)
  end

  it 'roots OAuth2 token-manager errors at the branded base' do
    token_err = Petstore::Client::Auth::OAuth::OAuth2TokenError.new('missing access_token')
    server_err = Petstore::Client::Auth::OAuth::OAuth2ServerError.new(400, 'invalid_grant', nil, nil, '{}')

    _(token_err).must_be_kind_of(::Petstore::Client::OpenAPIError)
    _(token_err).wont_be_kind_of(Petstore::Client::ApiError)
    _(server_err).must_be_kind_of(::Petstore::Client::OpenAPIError)
    _(server_err).wont_be_kind_of(Petstore::Client::ApiError)
  end
