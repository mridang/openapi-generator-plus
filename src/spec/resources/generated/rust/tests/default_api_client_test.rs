...
async fn test_default_api_client_multipart_body_replayed_on_307_redirect() {
    ...
    let resp = client
        .send_request(
            "POST",
            &format!("{}/api/redirect-307-multipart", wiremock_url),
            &HashMap::new(),
            Some(&body),
        )
        .await
    ...
    assert_eq!(
        json["replayed"], true,
        "redirect target must confirm body replay"
    );
}
...
async fn test_default_api_client_post_with_null_body_sends_content_length_zero() {
    ...
    let resp = client
        .send_request(
            "POST",
            &format!("{}/api/echo-content-length", wiremock_url),
            &headers,
            None,
        )
        .await
    ...
    assert_eq!(
        parsed.get("content-length").and_then(|v| v.as_str()),
        Some("0"),
