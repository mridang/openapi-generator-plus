...
// Gap AK: userinfo embedded in the proxy URL must reach the proxy as
// Proxy-Authorization. The Squid fixture's second port demands Basic proxy
// credentials: without them it answers 407, with them it forwards the request
// to the mock server.
#[tokio::test]
async fn test_default_api_client_proxy_with_credentials_injects_basic_authorization() {
    let chasm_url = testcontainers_helper::chasm_internal_http_url();
    let auth_proxy = testcontainers_helper::auth_proxy_url();
    let send = |proxy: String| async move {
        let transport = TransportOptionsBuilder::new()
            .proxy(&proxy)
            .build()
            .expect("valid transport options");
    ...
    let with_credentials = send(auth_proxy.replacen("http://", "http://user:pass@", 1)).await;
    assert_eq!(with_credentials.status_code(), 200);

    let without_credentials = send(auth_proxy.to_string()).await;
    assert_eq!(without_credentials.status_code(), 407);
}
...
async fn test_default_api_client_multipart_body_replayed_on_307_redirect() {
    ...
    fields.insert(
        ...
        petstore::api_client::MultipartValue::Text("hello".to_string()),
    );
    fields.insert(
        "file".to_string(),
        petstore::api_client::MultipartValue::Bytes(b"file-content-bytes".to_vec()),
    );
    let body = petstore::api_client::RequestBody::Multipart(fields);
...
async fn test_default_api_client_respects_max_redirects_limit() {
    ...
    let err = client
        ...
        .await
        .expect_err("a redirect beyond the cap must be refused");
    assert!(
