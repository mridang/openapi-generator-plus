...
async fn test_default_api_client_multipart_body_replayed_on_307_redirect() {
    ...
    let resp = client
        .send_request(
            "POST",
            &format!("{}/api/redirect-307", wiremock_url),
            &HashMap::new(),
            Some(&body),
        )
        .await
    ...
    assert!(
        echoed_body.contains("Content-Disposition: form-data; name=\"description\""),
        "redirect replay dropped the description part: {}",
        echoed_body
    );
    assert!(
        echoed_body.contains("Content-Disposition: form-data; name=\"file\""),
        "redirect replay dropped the file part: {}",
        echoed_body
    );
    assert!(
        echoed_body.contains("file-content-bytes"),
        "redirect replay dropped the file bytes: {}",
        echoed_body
    );
}
