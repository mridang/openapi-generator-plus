...
/// here, plus 301/303/308) rather than replaying the credential-bearing POST
/// to the redirect target. Like any non-2xx answer, the refusal is an
...
async fn test_token_endpoint_redirect_is_rejected() {
    for status in [301u16, 302, 303, 307, 308] {
        ...
        let server_err = err.downcast_ref::<OAuth2ServerError>().unwrap_or_else(|| {
            panic!("status {status} must surface as OAuth2ServerError, got: {err}")
        });
        assert_eq!(server_err.status_code(), status);
    }
}
