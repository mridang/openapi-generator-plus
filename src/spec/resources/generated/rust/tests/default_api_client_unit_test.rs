...
fn start_infinite_redirect_server(max_conns: usize) -> String {
    ...
    thread::spawn(move || {
        for (i, stream) in listener.incoming().enumerate() {
            ...
            // connection for each redirect hop. Without it the HTTP/1.1
            // default is keep-alive, and reqwest reuses the pooled
            // connection for the first redirect-follow — but this server
            // handles exactly one request per accepted connection (it loops
            // on `incoming().next()`), so the reused-connection request is
            // never read and the follow-up fails with a transport error
            // before the manual redirect loop can reach `max_redirects`.
            let response = format!(
                "HTTP/1.1 302 Found\r\nLocation: {}\r\nContent-Length: 0\r\nConnection: close\r\n\r\n",
                loc
            );
