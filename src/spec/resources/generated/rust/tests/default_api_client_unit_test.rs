...
/// The client attempts to inflate the body and fails.
...
/// uniform SDK error, NOT crash and NOT pass the corrupted bytes through
/// silently. The response arrived, so it is an `ApiError` carrying the real
/// status code — never a `NetworkError`.
#[tokio::test]
async fn test_al_content_encoding_lie_surfaces_api_error() {
    ...
    let err = client
        ...
        .await
        .expect_err("Gap AL: a Content-Encoding: gzip lie over a plaintext body must error");
    assert!(
        err.downcast_ref::<NetworkError>().is_none(),
        "Gap AL: a corrupt compressed body must not be a NetworkError, got: {}",
        err
    );
    let api_err = err
        .downcast_ref::<ApiError>()
        .unwrap_or_else(|| panic!("Gap AL: expected ApiError, got: {}", err));
    assert_eq!(
        api_err.status_code(),
        200,
        "the real status code must be kept"
    );
    assert!(
        std::error::Error::source(api_err).is_some(),
        "the decompression error must be kept as source()"
    );
    let _: &dyn OpenAPIError = api_err;
}
...
/// SDK NetworkError (statusCode 0), not surface as a raw reqwest::Error. Callers
/// downcast the boxed error to `NetworkError` to confirm the wrapping.
#[tokio::test]
async fn test_body_read_failure_wrapped_in_network_error() {
    ...
    assert!(
        ...
        "body-read failure must be wrapped in the SDK NetworkError type, got: {}",
        err
    );
}
...
/// refusal) must be wrapped in a NetworkError whose ApiError PRESERVES the
/// underlying reqwest error as its `source()`, rather than stringifying and
/// discarding it.
#[tokio::test]
async fn test_transport_error_preserves_underlying_cause() {
    ...
    let network_err = err
        ...
        .expect("transport failure must be a NetworkError");
    let _: &dyn OpenAPIError = network_err;
    let api_err = network_err.api_error();
    assert_eq!(api_err.status_code(), 0, "a transport failure has status 0");
    let source = std::error::Error::source(api_err)
        .expect("ApiError must retain the underlying transport error as source()");
    assert!(
        source.downcast_ref::<reqwest::Error>().is_some(),
        "the source must be the reqwest error, got: {}",
        source
    );
    assert!(
        matches!(
...
/// surfaces as NetworkTimeoutError with status 0, keeping the reqwest error.
#[tokio::test]
async fn test_timeout_returns_network_timeout_error() {
    let listener = TcpListener::bind("127.0.0.1:0").expect("failed to bind");
    let addr = listener.local_addr().expect("local addr");
    /* Accept the connection and never answer, so only the timeout ends it. */
    thread::spawn(move || {
    ...
    let err = result.expect_err("a request past the timeout must fail");
    let timeout_err = err
        .downcast_ref::<NetworkTimeoutError>()
        .unwrap_or_else(|| panic!("expected NetworkTimeoutError, got: {}", err));
    let _: &dyn OpenAPIError = timeout_err;
    ...
    assert!(
        std::error::Error::source(api_err).is_some(),
        "the timeout must keep the reqwest error as source()"
    );
    assert!(
        matches!(
            petstore::ApiErrorKind::from(api_err.clone()),
...
#[tokio::test]
async fn test_request_after_close_returns_invalid_state() {
    let base_url = start_method_echo_server();
    let client = DefaultApiClient::new(None);
    ...
    let err = client
        .send_request("GET", &format!("{}/echo", base_url), &headers, None)
        .await
        .expect_err("use-after-close must return an error, not succeed silently");
...
/// A malformed request URL is a caller mistake: `ConfigurationError::
/// InvalidArgument`, not a `NetworkError`.
#[tokio::test]
async fn test_malformed_url_returns_invalid_argument() {
    let client = DefaultApiClient::new(None);
    let err = client
        .send_request("GET", "http://[::1", &HashMap::new(), None)
        .await
        .expect_err("a malformed URL must fail");
    assert!(
        matches!(
            err.downcast_ref::<ConfigurationError>(),
            Some(ConfigurationError::InvalidArgument(_))
        ),
...
/// Starts a server that answers one connection with `response` verbatim and
/// returns its base URL.
fn start_canned_response_server(response: Vec<u8>) -> String {
    let listener = TcpListener::bind("127.0.0.1:0").expect("failed to bind");
    let addr = listener.local_addr().expect("local addr");
    thread::spawn(move || {
        if let Some(Ok(mut stream)) = listener.incoming().next() {
...
#[tokio::test]
async fn test_redirect_to_non_http_scheme_is_api_error_with_status() {
    let response = b"HTTP/1.1 302 Found\r\nLocation: ftp://example.com/file\r\nContent-Length: 0\r\nConnection: close\r\n\r\n".to_vec();
    let base_url = start_canned_response_server(response);
    ...
    let client = DefaultApiClient::new(Some(transport));
    let err = client
        .send_request("GET", &format!("{}/start", base_url), &HashMap::new(), None)
        .await
        .expect_err("a redirect to ftp:// must be refused");
    assert!(
        err.downcast_ref::<NetworkError>().is_none(),
        "must not be a NetworkError: {}",
        err
    );
    let api_err = err
        .downcast_ref::<ApiError>()
        .unwrap_or_else(|| panic!("expected ApiError, got: {}", err));
    assert_eq!(api_err.status_code(), 302);
}
...
/// falling back to the system trust store (security theater). The builder
/// returns `ConfigurationError::InvalidCaCertificate`: a configuration
/// mistake, not an `ApiError`, and not a panic.
#[test]
fn test_nonexistent_ca_cert_path_fails_fast() {
    ...
    /* ConfigurationError deliberately does not implement the SDK root trait:
     * `let _: &dyn OpenAPIError = &ConfigurationError::...` would not compile. */
}
