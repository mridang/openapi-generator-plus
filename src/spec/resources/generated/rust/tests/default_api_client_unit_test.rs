...
/// uniform SDK `NetworkError`, NOT crash and NOT pass the corrupted bytes through
/// silently. reqwest's gzip decoder fails to inflate the bogus body during the
/// body read; the client wraps that body-read failure as a `NetworkError`
...
async fn test_al_content_encoding_lie_surfaces_api_error() {
    ...
    assert!(
        ...
        "Gap AL: the decompression failure must be wrapped in the SDK NetworkError type, got: {}",
        err
    );
}
