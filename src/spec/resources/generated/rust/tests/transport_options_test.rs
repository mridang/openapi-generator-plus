...
fn test_transport_options_negative_timeout_is_rejected() {
    let result = TransportOptionsBuilder::new().timeout(Some(-1)).build();
    assert!(
...
/// "No timeout" must be expressible: `timeout(None)` clears the 10_000 ms
/// default rather than being indistinguishable from leaving it unset.
#[test]
fn test_transport_options_no_timeout_is_expressible() {
    let opts = TransportOptionsBuilder::new()
        .timeout(None)
        .build()
        .expect("valid transport options");
    assert_eq!(
        opts.timeout(),
        None,
        "expected timeout(None) to mean no timeout"
    );
}

/// `TransportOptions::builder()` is the static entry point every SDK exposes.
#[test]
fn test_transport_options_builder_entry_point() {
    let opts = TransportOptions::builder()
        .timeout(Some(2500))
        .build()
        .expect("valid transport options");
    assert_eq!(opts.timeout(), Some(2500));
    assert!(
        opts.verify_ssl(),
        "expected the same defaults as TransportOptionsBuilder::new()"
    );
}
