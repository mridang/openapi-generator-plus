        ...
        Box::pin(async move {
            ...
            if self.transport_options.follow_redirects() && !options.no_redirect {
                ...
                while is_redirect_status(response.status().as_u16()) && hops < max {
                    ...
                    response = redirect_builder.send().await.map_err(transport_error)?;
...
/// [`NetworkTimeoutError`]; any other failure (connection refused, DNS, TLS,
/// reset) [`NetworkError`]. Both carry status 0 and keep the reqwest error as
/// their source.
fn transport_error(e: reqwest::Error) -> Box<dyn std::error::Error + Send + Sync> {
    let message = e.to_string();
    if e.is_timeout() {
        let api_error = ApiError::transport(TransportFailure::Timeout, message, Arc::new(e));
        Box::new(NetworkTimeoutError::from(NetworkError::from(api_error)))
    } else {
        let api_error = ApiError::transport(TransportFailure::Network, message, Arc::new(e));
        Box::new(NetworkError::from(api_error))
    }
}
...
fn build_http_client(opts: &TransportOptions) -> Client {
    ...
    // TransportOptionsBuilder::build rejects a negative timeout, so the
    // conversion cannot fail; a failed one would otherwise wrap to an
    // effectively infinite timeout.
    if let Some(timeout_ms) = opts.timeout().and_then(|ms| u64::try_from(ms).ok()) {
...
mod tests {
    ...
    fn test_build_filename_directive_ascii_only_omits_filename_star() {
        ...
        assert_eq!(directive, "filename=\"file.png\"");
        assert!(
    ...
    fn test_validate_multipart_filename_rejects_crlf() {
        ...
        assert!(validate_multipart_filename("file.png").is_ok());
    }
    ...
    fn test_decode_text_body_bare_utf16_no_bom_is_big_endian() {
        ...
        // "Tag" as UTF-16BE without a BOM is 00 54 00 61 00 67.
        let big_endian_bytes = [0x00, 0x54, 0x00, 0x61, 0x00, 0x67];
        let decoded = decode_text_body(&big_endian_bytes, "text/plain; charset=utf-16");
        assert_eq!(decoded, "Tag");

        // The same bytes read little-endian would NOT be "Tag" (they decode to
        // U+5400 U+6100 U+6700), proving the big-endian choice is load-bearing.
    ...
    fn test_decode_text_body_utf16_honours_le_bom() {
        ...
        // select little-endian. "Tag" as UTF-16LE: 54 00 61 00 67 00.
        let le_with_bom = [0xFF, 0xFE, 0x54, 0x00, 0x61, 0x00, 0x67, 0x00];
    ...
    fn test_decode_text_body_utf16_honours_be_bom() {
        ...
        // "Tag" as UTF-16BE: FE FF 00 54 00 61 00 67.
        let be_with_bom = [0xFE, 0xFF, 0x00, 0x54, 0x00, 0x61, 0x00, 0x67];
