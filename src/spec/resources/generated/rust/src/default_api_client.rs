...
mod tests {
    ...
    // `MultipartValue::Text(object_serializer::serialize(&metadata)?)` — so the
    ...
    // with chrono's RFC 3339 wire form (millisecond fraction preserved). This
    // test reconstructs the operation's part-building path end-to-end through
    // `serialize_multipart_body` (the lowest-level multipart writer the
    ...
    fn test_multipart_model_part_uses_wire_property_names_and_datetime_format() {
        ...
        // call the generated add_pet_photos emits — then wrap it as the text
        // part the operation hands to the multipart writer.
        let part_json = object_serializer::serialize(&metadata)
            .expect("model part must serialize through the configured serializer");
        let mut fields = std::collections::HashMap::new();
        fields.insert("metadata".to_string(), MultipartValue::Text(part_json));
        let body = serialize_multipart_body(&fields, "BOUNDARY");
        let body_str = String::from_utf8_lossy(&body);
        ...
        // (3) The captured part is the text part for the `metadata` field —
