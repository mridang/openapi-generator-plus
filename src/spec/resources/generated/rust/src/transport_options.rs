        ...
        let ca_cert_pem = match &self.ca_cert_path {
            ...
            Some(path) => {
                let pem = std::fs::read(path).map_err(|e| {
                    ConfigurationError::InvalidCaCertificate(format!(
                        "failed to read {:?}: {}",
                        path, e
                    ))
                })?;
