...
impl OAuth2ImplicitAuthenticator {
    ...
    /// # Errors
    ///
    /// Returns [`ConfigurationError::InvalidArgument`] on invalid input and
...
impl Authenticator for OAuth2ImplicitAuthenticator {
    ...
    /// Returns the Bearer authentication header, or an empty map before
    /// [`set_access_token`](OAuth2ImplicitAuthenticator::set_access_token) is
    /// called. The API client calls
    /// [`try_auth_headers`](Self::try_auth_headers) instead, which reports the
    /// missing token.
    fn auth_headers<'a>(
        &'a self,
    ) -> Pin<Box<dyn Future<Output = HashMap<String, String>> + Send + 'a>> {
        Box::pin(async move { self.try_auth_headers().await.unwrap_or_default() })
    }

    /// Returns the Bearer authentication header, or
    /// [`ConfigurationError::InvalidState`] when no access token has been set.
