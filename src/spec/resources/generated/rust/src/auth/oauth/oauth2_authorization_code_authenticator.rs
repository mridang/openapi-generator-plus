...
impl OAuth2AuthorizationCodeAuthenticator {
    ...
    pub async fn exchange_code(
        ...
        if code.is_empty() || code.trim().is_empty() {
            return Err(Box::new(OAuth2AuthorizationCodeError::EmptyCode));
        }
...
impl Authenticator for OAuth2AuthorizationCodeAuthenticator {
    ...
    ///
    /// Returns an empty map if [`exchange_code`](Self::exchange_code) has not
    /// been called yet or if token refresh fails. The API client does not call
    /// this method: it calls [`try_auth_headers`](Self::try_auth_headers),
    /// which reports those failures instead.
    fn auth_headers<'a>(
        &'a self,
    ...
    /// ([`exchange_code`](Self::exchange_code) was never called, reported as
    /// [`OAuth2AuthorizationCodeError::CodeNotExchanged`]) or if token refresh
    /// failed.
    ...
    > {
        Box::pin(async move {
            ...
            if !token_exchanged {
                return Err(Box::new(OAuth2AuthorizationCodeError::CodeNotExchanged)
