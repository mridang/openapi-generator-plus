...
class TestContentLengthStrippedOnRedirect:
    ...
    def test_303_drops_content_length_when_body_is_dropped(self) -> None:
        ...
        assert not any(k.lower() == 'content-length' for k in followup_headers), f'Content-Length must be stripped on 303->GET; got {followup_headers!r}'
...
class TestRedirectExhaustion:
    ...
    def test_exhausting_redirect_budget_raises(self) -> None:
        ...
        transport = TransportOptions.builder().follow_redirects(True).max_redirects(3).build()
...
class TestCaCertPathFailsFast:
    ...
    def test_nonexistent_ca_cert_path_raises_api_exception(self) -> None:
        ...
        transport = TransportOptions.builder().ca_cert_path('/nonexistent/ca.pem').build()
        with pytest.raises(ApiException):
...
class TestApiKeyHeaderStrippedOnCrossOrigin:
    ...
    def test_api_key_header_stripped_on_cross_origin_redirect(self) -> None:
        ...
        api_key_header = api_key_header_names[0]
        ...
        assert not any(k.lower() == api_key_header.lower() for k in followup_headers), f'API-key header {api_key_header!r} leaked on cross-origin redirect: {followup_headers!r}'
    ...
    def test_authorization_stripped_on_cross_origin_redirect(self) -> None:
        ...
        assert len(pool.calls) == 2
        followup_headers = pool.calls[1][2]
        assert not any(k.lower() == 'authorization' for k in followup_headers), f'Authorization leaked on cross-origin redirect: {followup_headers!r}'
