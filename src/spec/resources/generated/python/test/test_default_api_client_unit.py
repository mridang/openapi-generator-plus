...
class TestApiKeyHeaderStrippedOnCrossOrigin:
    ...
    def test_api_key_header_stripped_on_cross_origin_redirect(self) -> None:
        ...
        assert not any(k.lower() == api_key_header.lower() for k in followup_headers), (
            f"API-key header {api_key_header!r} leaked on cross-origin redirect: {followup_headers!r}"
        )
