...
class TestInjectTraceContext:
    ...
    @pytest.mark.skip(reason='no ambient tracer like .NET Activity.Current; active-span injection requires a fully configured OpenTelemetry SDK out of scope for this unit test')
    def test_injects_traceparent_when_span_active(self) -> None:
        # .NET-specific scenario: relies on Activity.Current ambient context.
        ...

    @pytest.mark.skip(reason='no ambient tracer like .NET Activity.Current; tracestate-present requires a fully configured OpenTelemetry SDK out of scope for this unit test')
    def test_includes_tracestate_when_present(self) -> None:
        # .NET-specific scenario: relies on Activity.Current ambient context.
        ...

    @pytest.mark.skip(reason='no ambient tracer like .NET Activity.Current; empty-tracestate requires a fully configured OpenTelemetry SDK out of scope here')
    def test_omits_tracestate_when_empty(self) -> None:
        # .NET-specific scenario: relies on Activity.Current ambient context.
        ...

    @pytest.mark.skip(reason='no ambient tracer like .NET Activity.Current; trace-flags formatting requires a fully configured OpenTelemetry SDK out of scope for this unit test')
