...
def _list_adapter(inner: type) -> TypeAdapter[Any]:
    cached = _LIST_ADAPTER_CACHE.get(inner)
    if cached is None:
        cached = TypeAdapter(list[inner])  # type: ignore[valid-type]
        _LIST_ADAPTER_CACHE[inner] = cached
    return cached


def _dict_adapter(inner: type) -> TypeAdapter[Any]:
    cached = _DICT_ADAPTER_CACHE.get(inner)
    if cached is None:
        cached = TypeAdapter(dict[str, inner])  # type: ignore[valid-type]
