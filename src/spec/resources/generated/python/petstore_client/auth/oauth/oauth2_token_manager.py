...
class OAuth2TokenManager:
    ...
    def _fetch_token(
        ...
        Raises:
            ...
            OAuth2ServerException: If the token endpoint answers with a
                non-2xx status.
            OAuth2TokenException: If the token endpoint answers 2xx with an
                unusable body.
        """
        ...
        if response.status_code < 200 or response.status_code >= 300:
            ...
            # them into a typed OAuth2ServerException so callers can recover.
        ...
        # A 2xx body that is not a JSON object cannot yield a token: surface
        # it as OAuth2TokenException rather than a raw decoder error.
        try:
            token_data = json.loads(response.body)
        except ValueError as e:
            raise OAuth2TokenException(f"Token response is not valid JSON: {e}") from e
    ...
    @staticmethod
    def _parse_oauth2_server_error(
        status_code: int, body: str
    ) -> "OAuth2ServerException":
        """Parse an RFC 6749 §5.2 OAuth2 error response body into a typed
        :class:`OAuth2ServerException`. Falls back to a generic error using the
        raw body when the body is not a valid OAuth2 error object."""
        try:
            parsed = json.loads(body)
        except ValueError:
            return OAuth2ServerException(status_code, None, None, None, body)
...
class OAuth2ServerException(OpenAPIException):
