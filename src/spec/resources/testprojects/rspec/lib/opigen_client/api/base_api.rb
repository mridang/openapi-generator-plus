  ...
  class BaseApi
    ...
    def invoke_api(method, path, query_params, header_params, body, accepts, content_type, return_type)
      ...
      if response.status_code < 200 || response.status_code >= 300
        raise PetstoreClient::ApiError.new(code: response.status_code, response_body: response.body)
      end
    ...
    def build_query_string(query_params)
      query_params.reject { |_, v| v.nil? }.map { |k, v|
        ...
        if v.is_a?(Array)
          v.map { |val| "#{encoded_key}=#{CGI.escape(PetstoreClient::ObjectSerializer.to_query_value(val))}" }.join('&')
        else
          "#{encoded_key}=#{CGI.escape(PetstoreClient::ObjectSerializer.to_query_value(v))}"
        end
      }.join('&')
    end
